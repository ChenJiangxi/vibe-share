---
name: vibe-share
description: 帮个人开发者把刚做完的项目（vibe coding / demo / 开源 / 副业作品）写成中文社交平台（小红书 / B 站 / 抖音）发布包。自动读项目目录（README、git log、源码、设计稿），生成 SHARE.md：每个平台三个标题候选（文学钩子版 + 可检索关键词版 + 混合版）、SEO 关键词池、逐句配音稿、前 3 秒开场、封面构图、拍摄清单。创作者口吻不营销，但兼顾冷启流量 — 标题必含可搜索关键词，正文前 100 字让普通用户看懂作品是什么。触发词：发小红书 / 发 B 站 / 发抖音 / 写分享帖 / 发新项目 / 做发布包 / publish / post / 分享作品。
---
...
10. **禁止 100 字不落地**：正文前 100 字内**必须**出现一句话说清"它是什么形态 + 给谁用 + 能做什么"，用普通中文讲，不带术语。文学钩子开场没问题，但第 2–3 句必须落地。自测：一个从没听说过这个作品的陌生人读完前 100 字，能不能用自己的话复述它是什么？不能，就是你没写到。
11. **禁止圈内词裸用**：第一次出现"Claude Code""skill""commit message""vibe coding""prompt""pipeline""agent""MCP"这类普通社媒用户不懂的词时，**同段内必须带一句人话解释**（比如第一次写 Claude Code skill 要补一句"Claude Code 是命令行 AI 助手，skill 是它的插件"）。读者一卡在名词上就划走。
12. **禁止标题单轨**：每个平台的标题必须给 3 个候选（文学钩子版 + 关键词版 + 混合版），见《标题三选》。只给一个文学钩子版 = 放弃冷启流量 = 新号零曝光。
